...
@section('content')
    ...
    <div class="container-fluid px-4">
        ...
        <div class="container w-100 border border-3 border-secundary rounded p-4 mt-3">
            <form action="{{ route('pase_salida.update', ['pase_salida' => $pase_salida]) }}" method="post">
                @method('PATCH')
                @csrf
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <input type="text" id="nombre_alumno" class="form-control"
                            value="{{ $pase_salida->expedienteDisciplinario->alumno->matricula }} - {{ $pase_salida->expedienteDisciplinario->alumno->nombre }} {{ $pase_salida->expedienteDisciplinario->alumno->apellido }}"
                            readonly>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="numero_lista" id="numero_lista" class="form-control" value="{{ old('numero_lista', $pase_salida->numero_lista) }}" min="1" required>
                        @error('numero_lista')
                    ...
                    <div class="col-md-6">
                        ...
                        <select title="Seleccione un grado..." name="grado" id="grado" class="form-control selectpicker show-tick">
                            @foreach([1, 2, 3] as $grado)
                                <option value="{{ $grado }}" {{ old('grado', $pase_salida->grado) == $grado ? 'selected' : '' }}>
                                    {{ $grado }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="grupo" id="grupo" class="form-control">
                            ...
                            @foreach(['A', 'B', 'C', 'D', 'E', 'F'] as $grupo)
                                <option value="{{ $grupo }}" {{ old('grupo', $pase_salida->grupo) == $grupo ? 'selected' : '' }}>
                                    {{ $grupo }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="motivo" id="motivo" class="form-control">{{ old('motivo', $pase_salida->motivo) }}</textarea>
                        @error('motivo')
                        <small class="text-danger">{{ '*' . $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="time" name="hora_salida" id="hora_salida" class="form-control"
                               value="{{ old('hora_salida', date('H:i', strtotime($pase_salida->hora_salida))) }}" required>
                        @error('hora_salida')
                            <small class="text-danger">{{ '*' . $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="time" name="hora_regreso" id="hora_regreso" class="form-control"
                               value="{{ old('hora_regreso', date('H:i', strtotime($pase_salida->hora_regreso))) }}" required>
                        @error('hora_regreso')
                            <small class="text-danger">{{ '*' . $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="fecha_salida" id="fecha_salida" class="form-control" value="{{ old('fecha_salida', $pase_salida->fecha_salida) }}">
                        @error('fecha_salida')
                        <small class="text-danger">{{ '*' . $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input placeholder="solicito" type="text" name="solicito" id="solicito" class="form-control" value="{{ old('solicito', $pase_salida->solicito) }}">
                        @error('solicito')
                        <small class="text-danger">{{ '*' . $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 text-center">
                        ...
                        <a href="{{ route('pase_salida.index') }}" class="btn btn-secondary">Cancelar</a>
                    </div>
