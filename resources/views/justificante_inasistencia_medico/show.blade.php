...
<div class="container-fluid px-4">
    @if (!$fromExpedienteMedico) <!-- Mostrar solo si no viene del expediente -->
        ...
        <ol class="breadcrumb mb-4">
            ...
            <li class="breadcrumb-item"><a href="{{ route('justificante_inasistencia_medico.index') }}">Justificantes de inasistencia médica</a></li>
    ...
    <div class="container mt-4 border p-4">  
        ...
        <div class="ms-4">
            ...
            <p>
                Por este medio de la presente el servicio médico de la institución, le solicita de la manera más atenta y respetuosa que JUSTIFIQUE la inasistencia del alumno (a) <strong>{{ $justificante->expedienteMedico->alumno->nombre }} {{ $justificante->expedienteMedico->alumno->apellido }}</strong> del grado <strong>{{ $justificante->grado }}</strong> y grupo <strong>{{ $justificante->grupo }}</strong>, correspondiente al (os) módulos: <strong>{{ $justificante->modulos }}</strong> del día: <strong>{{ $justificante->fecha }}</strong>, ya que será retirado de la institución por problemas de salud.
            </p>
            <p>
